...
export function SearchPalette({ publishedCount }: { publishedCount: number }) {
  ...
  // One fetch attempt per open while docs are still null. Without this, a
  // failed load that leaves docs as null would re-fire the effect in a loop;
  // without resetting on close, a transient 404 (index not built yet) would
  // stick for the whole session even after the file appears.
  ...
  // public/search-index.json is gitignored and generated by `content:index`
  // (wired as predev/prebuild) — a missing file 404s, which must not be
  // remembered as "zero published lessons."
  useEffect(() => {
    if (!open) {
      fetchAttempted.current = false;
      return;
    }
    if (docs !== null || fetchAttempted.current) return;
              ...
              {docs === null && loadError ? (
                <div className="flex flex-col gap-2 p-4">
                  <p className="font-mono text-xs text-ink-muted">
                    Search index failed to load. Run{" "}
                    <code className="text-ink">npm run content:index</code> if
                    you are in local development.
                  </p>
                  <button
                    type="button"
                    onClick={retryLoad}
                    className="self-start rounded-card border-2 border-structural bg-card px-3 py-1.5 text-xs font-medium transition-brut hover:shadow-hard-sm"
                  >
                    Retry
                  </button>
                </div>
