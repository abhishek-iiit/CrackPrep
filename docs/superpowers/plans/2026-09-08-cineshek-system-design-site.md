...
**Files:**
- Create: `package.json`, `next.config.ts`, `postcss.config.mjs`, `tsconfig.json`, `vitest.config.ts`, `.gitignore`, `app/layout.tsx`, `app/page.tsx`, `app/globals.css`, `tests/setup.ts`, `tests/harness.test.ts`
- Modify: none
...
Create `tests/harness.test.ts`. This proves the runner, the path alias, and TSX transformation all work before any real code depends on them.
...
Rename the file to `tests/harness.test.tsx` — it contains JSX.
...
- Produces:
  - `progressStore` with `subscribe(cb): () => void`, `getSnapshot(): string`, `getServerSnapshot(): string`, `toggle(key: string): void`, `has(key: string): boolean`, `count(): number`, `clear(): void`
...
describe("progressStore", () => {
  ...
  it("survives unreadable storage", () => {
    ...
    expect(() => progressStore.clear()).not.toThrow();
    expect(progressStore.count()).toBe(0);
  });
...
`process.env.NEXT_PUBLIC_*` is inlined at build time, so this stays a Server Component with no client JavaScript.
...
- `progressStore` exposes the same seven members in Task 10's implementation, test, and `useProgress`.
